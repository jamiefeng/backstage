...
class QueryLogger
{
     ...
     *            optional, \Phalcon\Logger instance, defaults to Firephp adapter.
     * @param \Phalcon\Db\Profiler $profiler
     *            optional, \Phalcon\Db\Profiler instance
     * @param int $priority
     *            optional, defaults to \Phalcon\Logger::DEBUG
     */
    public function __construct(\Phalcon\Logger $logger = null, \Phalcon\Db\Profiler $profiler = null, $priority = \Phalcon\Logger::DEBUG)
    {
        $this->logger = $logger;
        if (! $this->logger) {
     ...
     *
     * @param int $priority
     *            priority level, one of \Phalcon\Logger constants
     *            
     * @return $this
     */
    ...
    public function afterQuery(\Phalcon\Events\Event $event, \Phalcon\Db\Adapter $connection)
    {
        ...
        $this->logger->log($this->priority, json_encode(array(
            'sql' => $this->profiler->getLastProfile()
                ->getSQLStatement(),
            'vars' => $this->profiler->getLastProfile()
                ->getSQLVariables(),
            'execution_time' => $this->profiler->getLastProfile()
                ->getTotalElapsedSeconds(),
            'bind_types' => $this->profiler->getLastProfile()
                ->getSQLBindTypes(),
            'host' => $host
        ), JSON_FORCE_OBJECT));
    }
