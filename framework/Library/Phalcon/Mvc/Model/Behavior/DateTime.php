...
class DateTime extends \Phalcon\Mvc\Model\Behavior
{
     ...
     *            Model instance which implements this behavior
     *            
     * @return bool|void on invalid event
     */
     ...
     *
     * @param \Phalcon\Mvc\Model $model
     *            model which implements this behavior
     * @param string $field
     *            field which contains \DateTime object
     *            
     * @return void
     *
    ...
    protected function onValidation(\Phalcon\Mvc\Model $model, $field)
    {
        if (! $model->readAttribute($field) instanceof \DateTime) {
            ...
            throw new \Phalcon\Mvc\Model\Behavior\Exception(sprintf('Property "%s" must be instance of DateTime object. "%s" given.', $field, $type));
        }
        $model->writeAttribute($field, $model->readAttribute($field)
            ->format('Y-m-d H:i:s'));
    }
     ...
     *            model which implements this behavior
     * @param string $field
     *            field which contains datetime string
     * @param string $timezone
     *            timezone string
     * @param string $className
     *            name of class on which "createFromFormat" method should be executed
     *            
     * @return void
     */
    protected function onAfterSave(\Phalcon\Mvc\Model $model, $field, $timezone, $className)
    {
        $model->writeAttribute($field, $className::createFromFormat('Y-m-d H:i:s', $model->readAttribute($field), new \DateTimeZone($timezone)));
    }
}
