...
class HandlerSocket extends Adapter implements AdapterInterface
{
    ...
    {
        $retval = $this->hs->executeSingle($this->hsIndex, '=', array(
            $id
        ), 1, 0);
    ...
    public function write($id, $data)
    {
        ...
        if (empty($this->fields)) {
            $this->hs->executeInsert($this->hsIndex, array(
                $id,
                date('Y-m-d H:i:s'),
                $data
            ));
        } else {
            $this->hs->executeUpdate($this->hsIndex, '=', array(
                $id
            ), array(
                $id,
                date('Y-m-d H:i:s'),
    ...
    {
        $this->hs->executeDelete($this->hsIndex, '=', array(
            $id
        ), 1, 0);
    ...
    public function gc($maxlifetime)
    {
        ...
        $this->hs->executeDelete($index, '<', array(
            $time
        ), 1000, 0);
