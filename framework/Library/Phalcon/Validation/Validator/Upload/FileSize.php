...
class FileSize extends \Phalcon\Validation\Validator
{
     ...
     *
     * @param mixed|\Phalcon\Http\Request $file
     *            uploaded file
     * @param null|int $min
    ...
    protected function checkSanity($file, $min = null, $max = null)
    {
        ...
        if ($min !== null && (int) $min <= 0) {
            throw new \UnexpectedValueException(sprintf('Option "min" cannot be zero or negative number. %d given.', $min));
        }
        
        if ($max !== null && (int) $max <= 0) {
            throw new \UnexpectedValueException(sprintf('Option "max" cannot be zero or negative number. %d given.', $max));
        }
    }
